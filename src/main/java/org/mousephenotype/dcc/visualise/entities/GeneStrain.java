...
@NamedQueries({
    @NamedQuery(name = "GeneStrain.all", query = "SELECT new org.mousephenotype.dcc.visualise.entities.GeneStrain(c.cid, c.gid, c.sid, g.geneSymbol, g.geneId, g.geneName, g.alleleName, s.strain, s.mgiStrainId, g.genotype, MAX(c.stateId.cid), SUM(c.numIssues - c.numResolved)) FROM DataContext c, ProcedureHasParameters phq, Parameter q, Genotype g, Strain s WHERE (c.pid = phq.procedureId.procedureId AND phq.parameterId.parameterId = q.parameterId AND c.qid = q.parameterId AND q.isMeta = 0 AND q.graphType IS NOT NULL AND c.gid = g.genotypeId AND g.genotypeId <> 0 AND c.sid = s.strainId AND c.numMeasurements > 0) GROUP BY c.cid, c.gid, c.sid ORDER BY s.strain, g.geneSymbol"),
    @NamedQuery(name = "GeneStrain.search", query = "SELECT new org.mousephenotype.dcc.visualise.entities.GeneStrain(c.cid, c.gid, c.sid, g.geneSymbol, g.geneId, g.geneName, g.alleleName, s.strain, s.mgiStrainId, g.genotype, MAX(c.stateId.cid), SUM(c.numIssues - c.numResolved)) FROM DataContext c, ProcedureHasParameters phq, Parameter q, Genotype g, Strain s WHERE (c.pid = phq.procedureId.procedureId AND phq.parameterId.parameterId = q.parameterId AND c.qid = q.parameterId AND q.isMeta = 0 AND q.graphType IS NOT NULL AND c.gid = g.genotypeId AND g.genotypeId <> 0 AND c.sid = s.strainId AND c.numMeasurements > 0 AND (g.geneSymbol LIKE :queryString OR g.alleleName LIKE :queryString OR s.strain LIKE :queryString)) GROUP BY c.cid, c.gid, c.sid ORDER BY s.strain, g.geneSymbol"),
    @NamedQuery(name = "GeneStrain.selected", query = "SELECT new org.mousephenotype.dcc.visualise.entities.GeneStrain(c.cid, c.gid, c.sid, g.geneSymbol, g.geneId, g.geneName, g.alleleName, s.strain, s.mgiStrainId, g.genotype, MAX(c.stateId.cid), SUM(c.numIssues - c.numResolved)) FROM DataContext c, ProcedureHasParameters phq, Parameter q, Genotype g, Strain s WHERE (c.pid = phq.procedureId.procedureId AND phq.parameterId.parameterId = q.parameterId AND c.qid = q.parameterId AND q.isMeta = 0 AND q.graphType IS NOT NULL AND c.gid IN :gids AND c.gid = g.genotypeId AND g.genotypeId <> 0 AND c.sid = s.strainId AND c.numMeasurements > 0) GROUP BY c.cid, c.gid, c.sid ORDER BY s.strain, g.geneSymbol")
})
